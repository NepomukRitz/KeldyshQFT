...
int main(int argc, char *argv[]){
	...
	Physics phy(N, Vg, h, mu, T, V_sg, pot_width);
...
#if RPA_MODE==0
	sprintf(filename,"dsfRG_mpi_qd_L%d_Lu%d_N%d_Nff_%d_NfbP_%d_NfbX_%d_num_freq_pre_%d_Vg_%f_h_%f_mu_%f_T_%f_U0_%f_U1_%f_Xi_%f_V_sg_%f_pot_width_%d_Lambda_ini_%f_Lambda_fin_%f_number_of_nodes_%d.mat",L,Lu,N,Nff,NfbP,NfbX,num_freq_pre,Vg,h,mu,T, U0,U1,Xi, V_sg, pot_width, Lambda_initial, sub_flow.resu(x_final), number_of_nodes);
#else
	sprintf(filename,"dsfRG_mpi_qd_rpa_L%d_Lu%d_N%d_Nff_%d_NfbP_%d_NfbX_%d_num_freq_pre_%d_Vg_%f_h_%f_mu_%f_T_%f_U0_%f_U1_%f_Xi_%f_V_sg_%f_pot_width_%d_Lambda_ini_%f_Lambda_fin_%f_number_of_nodes_%d.mat",L,Lu,N,Nff,NfbP,NfbX,num_freq_pre,Vg,h,mu,T, U0,U1,Xi, V_sg, pot_width,Lambda_initial, sub_flow.resu(x_final), number_of_nodes);
#endif
