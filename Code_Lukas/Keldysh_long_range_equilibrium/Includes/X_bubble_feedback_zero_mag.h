#ifndef X_BUBBLE_FEEDBACK_ZERO_MAG_27072017
#define X_BUBBLE_FEEDBACK_ZERO_MAG_27072017
...
template<int mode> int Integrand_X_bubble_feedback_zero_mag<mode>::number_of_eval=0; 
...
template <int mode> matrix<double> X_bubble_feedback_zero_mag<mode>::operator()(matrix<int> &job){
	return operator()(job(0), job(1));
}

template <int mode> int X_bubble_feedback_zero_mag<mode>::dim_r(matrix<int> &job){
	int l = job(0);
	return num.Nges - abs(l);
}

template <int mode> int X_bubble_feedback_zero_mag<mode>::dim_c(matrix<int> &job){
	int k = job(1);
	return num.Nges - abs(k);
}

template <int mode> int X_bubble_feedback_zero_mag<mode>::volume(matrix<int> &job){
