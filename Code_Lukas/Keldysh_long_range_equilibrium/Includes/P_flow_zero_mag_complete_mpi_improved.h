#ifndef P_FLOW_ZERO_MAG_COMPLETE_MPI_IMPROVED_23082017
#define P_FLOW_ZERO_MAG_COMPLETE_MPI_IMPROVED_23082017
...
#pragma omp parallel for
	for(int i=0; i<upper_boundary; ++i){
		...
		//syma<complex<double> > tmp(num.Nges);
		//tmp=(complex<double>) 0.0;
		
		for(int j1=0; j1<num.Nges; ++j1){
			for(int j2=0; j2<=j1; ++j2){
				int index = i*dim_syma + (j1*(j1+1))/2 + j2;
