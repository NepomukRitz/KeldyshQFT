#ifndef FLOW_ZERO_MAG_MPI_WITH_PAID_28072017
#define FLOW_ZERO_MAG_MPI_WITH_PAID_28072017
...
//#include "Vertex.h"
#include "Self_energy_central_zero_mag_with_paid_improved.h"
#include "P_flow_zero_mag_complete_mpi_improved_with_paid.h"
#include "XD_flow_zero_mag_complete_mpi_improved_with_paid.h"
#include "Norm.h"
...
template <int mode> class Flow_zero_mag{
	public:
		...
		Flow_zero_mag(Physics &phy_in, Numerics &num_in, Precomputation_zeromag<mode> &pre_in, Barevertex &barevertex_in, matrix<double> &additional_stops, list<matrix<double> > &performance_track);
...
template <int mode> void Flow_zero_mag<mode>::operator()(double x, Generalmatrix &y, Generalmatrix &dy){
	...
	Vertex<mode> dgamma(num,sub,dy);
 	//pre.precompute_fixed_grid_non_ps(Lambda,sub,gamma.ERetu_ipol_subst);
 	pre.precompute(Lambda,sub,gamma.ERetu_ipol_subst);
	
	auto start_time_p = std::chrono::system_clock::now();
	Integrand_P_bubble_paid<mode>::number_of_eval=0;	
	Integrand_P_bubble_feedback_zero_mag_with_paid<mode>::number_of_eval=0; 
	p_flow(Lambda, measure_flow, sub, gamma, dgamma, additional_stops); //additional_stops could also be changed during the flow!
	...
	Performance_tmp(2) = Integrand_P_bubble_paid<mode>::number_of_eval;
	Performance_tmp(3) = Integrand_P_bubble_feedback_zero_mag_with_paid<mode>::number_of_eval;

	auto start_time_x = std::chrono::system_clock::now();
	Integrand_X_bubble_paid<mode>::number_of_eval=0;	
	Integrand_X_bubble_feedback_zero_mag_with_paid<mode>::number_of_eval=0;
	xd_flow(Lambda, measure_flow, sub, gamma, dgamma, additional_stops);
	...
	Performance_tmp(5) = Integrand_X_bubble_paid<mode>::number_of_eval;
	Performance_tmp(6) = Integrand_X_bubble_feedback_zero_mag_with_paid<mode>::number_of_eval;
	
#if RPA_MODE ==0	
	
	auto start_time_self = std::chrono::system_clock::now();
	Integrand_self_energy_dyn_central_zero_mag_with_paid<mode>::number_of_eval=0; 
	Integrand_S_keldysh_zero_mag_with_paid<mode>::number_of_eval=0; 

	/*Static Contribution to selfenergy:*/
	Self_energy_static_zero_mag_with_paid<mode> self_stat(phy,num,pre,sub,Lambda,measure_flow,gamma,barevertex, ACCURACY_S_BUB, PAID_ORDER, additional_stops);
	time(&t1);
	...
	cout<<"Compute Dynamic Self_energy part"<<endl;	
	time(&t1);
	Self_energy_dynamic_zero_mag_with_paid<mode> self_dyn(phy,num,pre,sub,Lambda,measure_flow,gamma, ACCURACY_S_BUB, PAID_ORDER, additional_stops);
...
#pragma omp parallel for
	for(int i=0; i<upper_boundary; ++i){
		...
		//syma<complex<double> > tmp = Self_stat;
...
#pragma omp parallel for
	...
	Performance_tmp(8) = Integrand_self_energy_dyn_central_zero_mag_with_paid<mode>::number_of_eval;
	Performance_tmp(9) = Integrand_S_keldysh_zero_mag_with_paid<mode>::number_of_eval;
