...
int main(int argc, const char *argv[]){
	...
	syma<double> density_reduced(num.Nges-1);
	syma<double> ldos_reduced(num.Nges-1);
	for(int i=0; i<num.Nges-1; ++i){
	 	for(int j=0; j<=i; ++j){
		 	density_reduced(i,j) = density(i,j);
		 	ldos_reduced(i,j) = ldos(i,j);
		}
	}

	matrix<matrix<double> > Rotation =density_reduced.eig(); 
	...
	char savename[10000];
	sprintf(savename,"density_matrix_tmp_N%d_Vg%f_mu%f_T%f_U0%f_num_freq_pre%d.mat",N,Vg,mu,T,U0,num_freq_pre);	
	ldos_reduced.save(savename,"ldos_integrated");
	density_reduced.save(savename,"density");
	Eigenvalues.save(savename,"eig_density");
	num.save(savename);
	phy.save(savename);
	syma<double> hamiltonian(num.Nges-1);
	for(int i=0; i<num.Nges-1; ++i){
	 	for(int j=0; j<=i; ++j){
		 	hamiltonian(i,j) = real(phy.hamiltonian(i,j));
		}
	}
	hamiltonian.save(savename,"hamiltonian");
