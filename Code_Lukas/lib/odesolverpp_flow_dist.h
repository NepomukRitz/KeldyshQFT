#ifndef __ODESOLVER_FLOW_DIST_H__
#define __ODESOLVER_FLOW_DIST_H__
#define MAXSTP 10000
#include <matrix.h>
#include <iostream>
...
int odeint3(yT& ystart,xT x1,xT x2,xT eps,xT atol,xT rtol,
             xT h1,xT hmin,long& nok, long& nbad,dT &derivs, bool compute_additional_data_after_each_step=false)
{
	long i,nstp;
	yT y,dydx;
	xT x,h,hnext,hdid;
	...
	for (nstp=1;nstp<=MAXSTP;nstp++)
	{
		...
		*/
		...
		/* if we want to compute more things, call routine compute_additional_data_after_each_step from derivs object. This function gets told about all currently available info: value of the vector, value of the derivative, and current value of the flow parameter */
		if (compute_additional_data_after_each_step)
		{
			derivs.compute_additional_data_after_each_step(y, dydx, x);
		}
