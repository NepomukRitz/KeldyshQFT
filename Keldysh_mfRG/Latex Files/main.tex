...
\section*{General properties of the Green's function}
\begin{equation}
...
Within the Keldysh formalism, the propagator has three different non-zero components. Due to their analytical properties (considered as functions of a complex time variable $t\in \mathbb{C}$), these are called \textit{retarded}, \textit{advanced}, and \text{Keldysh}. They are defined as follows:
\begin{equation}
...
The self energy behaves in, essentially, the same way as the propagator i.e is diagonal in frequency- (when time-translational symmetry), momentum- (when space-translational symmetry) and spin-space. It also has two independent components, $\Sigma^R$, which it's conjugate is $\Sigma^A = \left[\Sigma^R\right]^*$ and $\Sigma^K$, which is anti-hermitian, i.e. $\left[\Sigma^K\right]^* = -\Sigma^K$. Due to $G^{1|1}=0$, it directly follows that $\Sigma^{1|1}=0$.
...
\begin{align}
  ...
  \end{array}
\end{align}
The inclusion of $T_S$ expands the group, but does not yield any interesting structure.
...
i_1^p (\alpha'_1\alpha'_2|\alpha_1\alpha_2, \alpha'_3\alpha'_4|\alpha_3\alpha_3) &= \alpha'_1\alpha'_2| \alpha'_3\alpha'_4 \\
i_3^p (\alpha'_1\alpha'_2|\alpha_1\alpha_2, \alpha'_3\alpha'_4|\alpha_3\alpha_3) &= \alpha_3\alpha_4| \alpha_1\alpha_2 \\
