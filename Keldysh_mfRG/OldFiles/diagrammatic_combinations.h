...
auto left_same_bare (const Vertex<Q>& vertex, int i1, double w, double vpp, int i_in, int spin, char channel) -> Q
{
    ...
    switch (channel){
        case 'a':
#if DIAG_CLASS >=1
            K1 = vertex[0].avertex.template valsmooth<k1>(input, vertex[0].tvertex);
#endif
#if DIAG_CLASS >=2
            K2b = vertex[0].avertex.template valsmooth<k2b>(input, vertex[0].tvertex);
#endif
            break;

        case 'p':
#if DIAG_CLASS >=1
            K1 = vertex[0].pvertex.template valsmooth<k1>(input);
#endif
#if DIAG_CLASS >=2
            K2b = vertex[0].pvertex.template valsmooth<k2b>(input);
#endif
            break;
        case 't' :
#if DIAG_CLASS >=1
            K1 = vertex[0].tvertex.template valsmooth<k1>(input, vertex[0].avertex);
#endif
#if DIAG_CLASS >=2
            K2b = vertex[0].tvertex.template valsmooth<k2b>(input, vertex[0].avertex);
#endif
            break;
        default:
            return 0.;
    ...
    switch (channel) {
        case 'a':
            K1 += vertex[0].pvertex.template valsmooth<k1>(input_p)
                  + vertex[0].tvertex.template valsmooth<k1>(input_at, vertex[0].avertex);
            break;
        case 'p':
            K1 += vertex[0].avertex.template valsmooth<k1>(input_at, vertex[0].tvertex)
                  + vertex[0].tvertex.template valsmooth<k1>(input_at, vertex[0].avertex);
            break;
        case 't':
            K1 += vertex[0].avertex.template valsmooth<k1>(input_at, vertex[0].tvertex)
                  + vertex[0].pvertex.template valsmooth<k1>(input_p);
            break;
        default: ;
    }
#endif
...
#endif
#if DIAG_CLASS >=2
            K2 = vertex[0].avertex.template valsmooth<k2>(input, vertex[0].tvertex);
#endif
            break;
...
#endif
#if DIAG_CLASS >=2
            K2 = vertex[0].pvertex.template valsmooth<k2>(input);
#endif
            break;
        case 't' :
#if DIAG_CLASS >=1
...
#endif
#if DIAG_CLASS >=2
            K2 = vertex[0].tvertex.template valsmooth<k2>(input, vertex[0].avertex);
#endif
            break;
        default:
            return 0.;
...
#endif
#if DIAG_CLASS >=3
            K3 = vertex[0].avertex.template valsmooth<k3>(input, vertex[0].tvertex);
#endif
            break;
        case 'p':
#if DIAG_CLASS >=2
...
#endif
#if DIAG_CLASS >=3
            K3 = vertex[0].pvertex.template valsmooth<k3>(input);
#endif
            break;
        case 't':
#if DIAG_CLASS >=2
...
#endif
#if DIAG_CLASS >=3
            K3 = vertex[0].tvertex.template valsmooth<k3>(input, vertex[0].avertex);
#endif
            break;
        default:
            return 0.;
