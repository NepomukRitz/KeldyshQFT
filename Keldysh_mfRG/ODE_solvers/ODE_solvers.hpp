 ...
 */
    ...
    void rkqs(butcher_tableau<stages> tableau, Y &state_i, double &Lambda_i, double htry, double &hdid, double &hnext,
              ...
              const std::vector<double> &lambda_checkpoints, const System& rhs, size_t iteration, const ODE_solver_config& config, const bool verbose)
    {
        ...
        for (;;)    // infinite loop
        {
            ...
            if (attempts > config.max_stepResizing_attemps and tableau.adaptive) {
...
void ode_solver(Y& result, const double Lambda_f, const Y& state_ini, const double Lambda_i, const System& rhs,
                std::vector<double> lambda_checkpoints = {}, std::string filename = "", const ODE_solver_config& config=ODE_solver_config_standard, const bool verbose=true) {
    int world_rank = mpi_world_rank();
    ...
    const int MAXSTP = config.maximal_number_of_ODE_steps + lambda_checkpoints.size(); //maximal number of steps that is computed
    ...
    const vec<double> lambdas_try = flowgrid::construct_flow_grid(Lambda_f, Lambda_i, FlowGrid::t_from_lambda, FlowGrid::lambda_from_t, config.maximal_number_of_ODE_steps, lambda_checkpoints);
    ...
    for (size_t i = config.iter_start; i < MAXSTP; i++)
    {
        ...
        ode_solver_impl::rkqs<Y, FlowGrid>(tableau, result, Lambda, h, hdid, hnext, min_t_step, max_t_step, lambda_checkpoints, rhs, i, config, verbose);
        ...
        namespace odeint {
            namespace detail {
                ...
                State_t integrate_nonadaptive(
                        ...
                        std::string filename, const ODE_solver_config& config, const bool verbose
                )
                {
                ...
                )
                {
                    ...
                    const size_t max_attempts = config.max_stepResizing_attemps;
                    ...
                    while( less_with_sign( t_now, t_final, dt ))
                    {
                        ...
                        while( ( res == fail ) && ( trials < config.max_stepResizing_attemps ) );
                        if( trials == config.max_stepResizing_attemps ) throw std::overflow_error( error_string );
...
#define ERR_STEPPER runge_kutta_dopri5
                ...
                const size_t MAXSTP = config.maximal_number_of_ODE_steps + lambda_checkpoints.size(); //maximal number of steps that is computed
                // get lambdas according to FlowGrid (for hybridization flow: + checkpoints acc. to U_NRG  ) -> for non-adaptive method
                ...
                result = detail::integrate_nonadaptive<error_stepper_t, State_t, FlowGrid>(
                        ...
                        t_now, t_final, lambdas_try, lambdas_did, filename, config, verbose); //, typename Stepper::stepper_category() );
#else
                typedef ERR_STEPPER< State_t, double, State_t, double, boost::numeric::odeint::vector_space_algebra > error_stepper_t;
                ...
                result = detail::integrate_adaptive_check<controlled_error_stepper_t, State_t, FlowGrid>(
                        ...
                        t_now, t_final, dt, MAXSTP, lambda_checkpoints, lambdas_did, filename, config, verbose);
