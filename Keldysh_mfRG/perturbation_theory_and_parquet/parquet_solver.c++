...
void parquet_checks(const std::string filename, const fRG_config& config) {
    ...
    for (unsigned int i=0; i<Lambdas.size(); ++i) {
        ...
        compute_BSE(Gamma_BSE, state, Lambdas[i], config, i);       // compute the lhs of the BSE
        ...
        compute_SDE(Sigma_SDE, state, Lambdas[i], config, 1);               // compute the lhs of the SDE
...
void run_parquet(const fRG_config& config, const std::vector<double>& U_NRG_list, const int version, const bool overwrite_old_results){
    ...
    for (double Lambda : Lambda_checkpoints) {
        ...
        state.initialize();
        sopt_state(state, Lambda, config);
        const double Delta = (config.Gamma + Lambda) * 0.5;
        double U_over_Delta = config.U / Delta;
        const std::string parquet_filename = data_dir + "parquetInit4_T=" + std::to_string(config.T) + "_U_over_Delta=" + std::to_string(U_over_Delta) + "_n1=" + std::to_string(nBOS) + "_n2=" + std::to_string(nBOS2) + (MAX_DIAG_CLASS > 2 ? "_n3=" + std::to_string(nBOS3) : "") + ".h5" + "_SDEv" + std::to_string(version); // (VECTORIZED_INTEGRATION ? "_vecd" : "_nonvecd");
//state = read_state_from_hdf(parquet_filename, 30);
        parquet_solver(parquet_filename, state, Lambda, version, 1e-6, 2, overwrite_old_results);
    }
}