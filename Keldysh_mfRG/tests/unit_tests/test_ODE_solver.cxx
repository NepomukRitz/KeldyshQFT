...
TEST_CASE( "Does the ODE solver work for a simple ODE?", "[ODEsolver]" ) {
    ...
    boost::numeric::odeint::ode_solver_boost<double, flowgrid::linear_parametrization, rhs_quartic_t>(result, Lambda_f, y_ini, Lambda_i, rhs_quartic_t(), lambda_checkpoints, "", 0, 1, false);


    double result_exact = 0.;
    SECTION( "Is the correct value retrieved from ODE solver?" ) {
        REQUIRE( std::abs(result - result_exact) < 1e-10 );
    }
