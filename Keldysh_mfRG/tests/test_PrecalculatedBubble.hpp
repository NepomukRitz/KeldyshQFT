...
template<typename Q> double test_PrecalculateBubble<Q>::find_largest_deviation_from_bubble(const bool dot_in, const char channel){
    ...
    for (int iK = 0; iK < glb_number_of_Keldysh_components_bubble; ++iK) {
        for (int iw = 0; iw < nBOS; ++iw) {
            const double w = g.selfenergy.Sigma.frequencies.  primary_grid.get_frequency(iw);
            for (int ivpp = 0; ivpp < nFER; ++ivpp) {
                const double vpp = g.selfenergy.Sigma.frequencies.  primary_grid.get_frequency(ivpp);
...
write_h5_rvecs(filename,
               {"Frequencies",
                ...
                "AbsoluteDeviations"},
               {g.selfenergy.Sigma.frequencies.  primary_grid.all_frequencies,
