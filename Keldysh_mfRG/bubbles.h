...
public:
    ...
    auto operator() (double vpp) const -> comp {
        ...
        return Pi.value(iK, v1, v2)/(2.*pi*glb_i);
    }
};
...
#pragma omp parallel for
            for (int i_omp=0; i_omp<n_omp; ++i_omp) {
                ...
                if(diff){
                    ...
                    value = prefactor*(1./(2.*pi*glb_i))*integrator(integrand_K1, w_lower_f, w_upper_f);                      //Integration over a fermionic frequency
                } //TODO: prefactor -1./(2.*pi*glb_i) into Integrand classes?
                else{
                    Integrand_K1<Q> integrand_K1(vertex1, vertex2, Pi, i0, w, i_in, channel);
                    value  = prefactor*(1./(2.*pi*glb_i))*integrator(integrand_K1, w_lower_f, w_upper_f);                      //Integration over a fermionic frequency
                    value += prefactor*(1./(2.*pi*glb_i))*asymp_corrections_K1(vertex1, vertex2, w_upper_b, w_upper_b, w, i0, i_in, channel); //Correction needed for the K1 class
                }
...
#pragma omp parallel for
            for (int i_omp=0; i_omp<n_omp; ++i_omp) {
                ...
                if(diff){
                    ...
                    value = prefactor*(-1./(2.*pi*glb_i))*integrator(integrand_K2, w_lower_f, w_upper_f);                      //Integration over vppp, a fermionic frequency
                }
                else{
...
#pragma omp parallel for
            for (int i_omp=0; i_omp<n_omp; ++i_omp) {
                ...
                if (diff){
                    ...
                    value = prefactor*(-1./(2.*pi*glb_i))*integrator(integrand_K3, w_lower_f, w_upper_f);                      //Integration over vppp, a fermionic frequency                    // y
                }
                else{
                    Integrand_K3<Q> integrand_K3 (vertex1, vertex2, Pi, i0, w, v, vp,  i_in, channel, part);

                    value = prefactor*(-1./(2.*pi*glb_i))*integrator(integrand_K3, w_lower_f, w_upper_f);                      //Integration over vppp, a fermionic frequency
                }
