...
protected:
    //std::vector<double> m_x = DataContainer::frequencies.b.auxiliary_grid;
...
{
    size_t n_x = DataContainer::frequencies.b.get_all_frequencies().size();
    size_t n_nonx = n/n_x;
    ...
    for(size_t i=0; i<n_nonx; i++) {
        for (size_t j=0; j<n_x-1; j++) { /// i=n_x-1 not treated (only used for extrapolation to the right)
            const double h  = DataContainer::frequencies.b.get_auxiliary_gridpoint(j+1)-DataContainer::frequencies.b.get_auxiliary_gridpoint(j);      /// spacing
...
void Spline<Q,rank,1,pos_first_freq_index,DataContainer>::initInterpolator() const
{
    ...
    auto Spline<Q,rank,1,pos_first_freq_index,DataContainer>::get_weights (int idx, double t) const -> weights_type{
        ...
        double t_low = DataContainer::frequencies.b.get_auxiliary_gridpoint(idx);
        double t_high= DataContainer::frequencies.b.get_auxiliary_gridpoint(idx+1);
        double h = (t - t_low);
        //assert(h>-1e-10);
...
result_type Spline<Q,rank,1,pos_first_freq_index,DataContainer>::interpolate_spline (const frequencies_type& frequencies,const index_type& indices) const //int iK, int ispin, double x, int i_in
{
    assert(initialized);
    double t;
    int idx=DataContainer::frequencies.b.get_grid_index(t, frequencies[0]);
    ...
    double h = t - DataContainer::frequencies.b.get_auxiliary_gridpoint(idx);
    ...
    {
    assert(order>0);
    size_t idx = DataContainer::frequencies.b.get_grid_index(x);
