...
void compare_flow_with_FDTs(const std::string filename, bool write_flag) {
    ...
    compare_with_FDTs(state_in.vertex, Lambda_ini, Lambda_int, filename, write_flag, nLambda);

    for (unsigned int i = 1; i < nLambda; i++) {
        state_in = read_state_from_hdf(filename, i); // read state
        compare_with_FDTs(state_in.vertex, Lambdas[i], i, filename, write_flag, nLambda);
    }
}
