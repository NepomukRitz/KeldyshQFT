...
void IdentityChecker::compute_1D_WardIdentity_RHS() const {
    const Propagator<comp> G (NRG_state.Lambda, NRG_state.selfenergy, 'g', NRG_state.config);
...
void IdentityChecker::compute_2D_WardIdentity_LHS() const {
    ...
    for (int iw=0; iw<nBOS; ++iw){
        ...
        for (int iv=0; iv<nFER; ++iv){
            const double v = NRG_state.selfenergy.Sigma.frequencies.get_freqGrid_b().get_frequency(iv);
            const comp left_term  = value_of_Sigma_for_LHS(NRG_state.selfenergy, v + w, a1p, a1_bar);
            const comp right_term = value_of_Sigma_for_LHS(NRG_state.selfenergy, v    , a1p_bar, a1);
...
/*comp value_of_Sigma_for_LHS(const SelfEnergy<comp>& Sigma, const double vt, const int k1p, const int k1){
    if ((k1p == 0) and (k1 == 0)) return Sigma.valsmooth(1, vt, 0);
    if ((k1p == 0) and (k1 == 1)) return Sigma.valsmooth(0, vt, 0);
    if ((k1p == 1) and (k1 == 0)) return conj(Sigma.valsmooth(0, vt, 0));
    if ((k1p == 1) and (k1 == 1)) return 0.0;
...
std::vector<std::vector<comp>> evaluate_2D_WardIdentity_LHS(const State<comp,false>& NRG_state){
    ...
    for (int iw=0; iw<nBOS; ++iw){
        ...
        for (int iv=0; iv<nFER; ++iv){
            ...
            WI_LHS[iv] = glb_i * (left_term - right_term);
        }
        results.push_back(WI_LHS);
    }
    return results;
}*/
