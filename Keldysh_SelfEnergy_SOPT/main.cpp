...
const long int n_integration = 3;
//Number of evolution steps
const int n_evolution = 5;

//Frequency range on which it's integrated
const double x_a = -2.0;
const double x_b = 2.0;
const double domega = (x_b-x_a)/((double)(n_integration));
...
Propagator GR(double l, Propagator &SigmaR)
{
    Propagator resp(omega.size());
    for (int i=0; i<omega.size();i++) {
        resp[i] = 1. / (omega[i] - epsilon + (comp)1.i*(Gamma + l)/2. - SigmaR[i]);
    }
    return resp;
}
...
Propagator GA(double l, Propagator &SigmaA)
{
    Propagator resp(omega.size());
    for (int i=0; i<omega.size();i++) {
        resp[i] = 1. / (omega[i] - epsilon - (comp)1.i*(Gamma + l)/2. - SigmaA[i]);
    }
    return resp;
}
...
Propagator derivative_Sigma22(double lam)
{
    ...
    Propagator resp(omega.size()), integrand(omega.size()), summand1(omega.size()), summand20(omega.size()), summand3(omega.size());
    ...
    for(int i=0; i<omega.size(); i++){
        for (int j=0; j<omega.size(); j++){
            comp sr = sR(lam,omega[j]);
            comp sa = sA(lam,omega[j]);
            comp sk = sK(lam,omega[j]);

            summand1[j] = vertex1[i][j][i]*sr;
            summand20[j] = vertex2[i][j][i]*sa;
            summand3[j] = vertex3[i][j][i]*sk;
        }

        integrand = summand1+summand20+summand3;
