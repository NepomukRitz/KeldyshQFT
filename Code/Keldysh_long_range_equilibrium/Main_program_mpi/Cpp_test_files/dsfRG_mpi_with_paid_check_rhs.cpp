...
int main(int argc, char *argv[]){
	...
	for(int i=0; i<num.Nff; ++i){
		gamma_data.short_str(7)(i)  = phy.hamiltonian; 
		gamma_data.short_str(8)(i)  = phy.hamiltonian; 
	}

	//Test save	
	char filename_save[255];
	sprintf(filename_save,"dsfRG_mpi_with_paid_check_rhs_L%d_Lu%d_N%d_Nff_%d_NfbP_%d_NfbX_%d_num_freq_pre_%d_Vg_%f_h_%f_mu_%f_T_%f_U0_%f_U1_%f_Xi_%f_Lambda_ini_%f_Lambda_fin_%f_number_of_nodes_%d.mat",L,Lu,N,Nff,NfbP,NfbX,num_freq_pre,Vg,h,mu,T, U0,U1,Xi,Lambda_initial, sub_flow.resu(x_final), number_of_nodes);
	...
	cout<<"Testdiff="<<dgamma_data.short_str(7)(660)(4,4) - dgamma_data.short_str(7)(659)(4,4)<<endl;
