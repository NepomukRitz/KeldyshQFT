...
int main(){
	...
	//Test:
	{
		...
		double freq = 0.5;
		double T=0.1;
		matrix<double> b_pre = b_prefactor_x(wb,T);
		b_pre = wb;
		b_pre(0) = 2;
		cout<<"b_pre="<<b_pre<<endl;
		matrix<matrix<complex<double> > > C = Str.dyn_ipol_b(freq,pos_feedback,b_pre);  
		cout<<"C="<<endl;
		cout<<C<<endl;
		matrix<matrix<complex<double> > > D = Str.complete_ipol_b(freq,pos_feedback,L_bounds,b_pre);  
		cout<<"D="<<endl;
		cout<<D<<endl;
	}
