...
int main(){
	...
	Barevertex barevertex(N,Lu,U1,U2,Xi);
	{
		...
		Integrator_self<mode, Integrand_self_opposite_spin<mode> > integrator_oppo(spin, phy, num, pre, sub, measure_flow, gamma, accuracy, additional_stops, stops_obj);	
		syma<complex<double> > A = integrator_same(external_freq);
		syma<complex<double> > B = integrator_oppo(external_freq);
		cout<<"A="<<endl;
		cout<<A<<endl;
		cout<<"B="<<endl;
		cout<<B<<endl;
