...
int main(int argc, char *argv[]){
	...
	double Vs=get_option(argc,argv,"Vs");
	...
	Physics phy(N, Vg, h, mu, T, Vs, pot_width);
	...
	char filename[10000];
		...
		#if RPA_MODE==0
	        sprintf(filename,"D_L%d_Lu%d_N%d_Nff%d_NfbP%d_NfbX%d_pre%d_NL%d_Vg%.4f_h%f_mu%.4f_T%f_Uc%.2f_Uo%.2f_Xi%.2f_Vs%.4f_pw%d_ap%.0e_ax%.0e_as%.0e_tol%.0e_Li%.1f_Lf%.1f_no%d.mat",L,Lu,N,Nff,NfbP,NfbX,num_freq_pre,NL_full,Vg,h,mu,T,U0,U1,Xi,Vs,pot_width,accuracy_p,accuracy_x,accuracy_self,tolerance,Lambda_initial,Lambda_final, number_of_nodes);
		#else
	        sprintf(filename,"D_rpa_L%d_Lu%d_N%d_Nff%d_NfbP%d_NfbX%d_pre%d_NL%d_Vg%.4f_h%f_mu%.4f_T%f_Uc%.2f_Uo%.2f_Xi%.2f_Vs%.4f_pw%d_ap%.0e_ax%.0e_as%.0e_tol%.0e_Li%.1f_Lf%.1f_no%d.mat",L,Lu,N,Nff,NfbP,NfbX,num_freq_pre,NL_full,Vg,h,mu,T,U0,U1,Xi,Vs,pot_width,accuracy_p,accuracy_x,accuracy_self,tolerance,Lambda_initial,Lambda_final, number_of_nodes);
		#endif
		//#if(ADD_KATANIN==1)
	...
	if(rank==root){	
		...
		tmp(0) =Vs;
		tmp.save(filename,"Vs");
		tmp(0) =pot_width;
		tmp.save(filename,"pot_width");
