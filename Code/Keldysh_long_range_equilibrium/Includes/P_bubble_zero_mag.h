#ifndef P_BUBBLE_ZERO_MAG_15032017
#define P_BUBBLE_ZERO_MAG_15032017
...
template <int mode> class Integrand_P_bubble_zero_mag{
	public:
		...
		syma<complex<double> > Gu_diff_syma, Su_syma; 
		...
		Integrand_P_bubble_zero_mag(double external_freq, Physics &phy_in, Numerics &num_in, Precomputation_zeromag<mode> &pre_in, Substitution<mode> &sub_in, double measure_flow_in, int l, int k);
		matrix<complex<double> > operator()(double internal);
		matrix<double> select(matrix<complex<double> > &M);
};

template<int mode> Integrand_P_bubble_zero_mag<mode>::Integrand_P_bubble_zero_mag(double external_freq_in, Physics &phy_in, Numerics &num_in, Precomputation_zeromag<mode> &pre_in, Substitution<mode> &sub_in, double measure_flow_in, int l_in, int k_in): external_freq(external_freq_in), phy(phy_in), num(num_in), pre(pre_in), sub(sub_in), measure_flow(measure_flow_in), l(l_in), k(k_in), jmin(max(0,-l)), jmax(min(num.twoN,num.twoN-l)), imin(max(0,-k)), imax(min(num.twoN,num.twoN-k)), Gu_diff_syma(num.Nges), Su_syma(num.Nges){} 
 
template<int mode> matrix<complex<double> > Integrand_P_bubble_zero_mag<mode>::operator()(double internal){
	...
	for(int jsum=jmin+l, jtilde=0, j=jmin; j<=jmax;++j, ++jsum, ++jtilde){ 
		for(int isum=imin+k, itilde=0, i=imin; i<=imax;++i, ++isum, ++itilde){  
			...
			ret(jtilde,itilde) = nf* ( 
			                          Su_matrix(j,i).imag()*Gu_diff_matrix(jsum,isum)
			                         )
			                   + nfm*(
			                          Gu_diff_matrix(j,i).imag()*Su_matrix(jsum,isum)
			                         );
#else
			ret(jtilde,itilde) = nf* ( 
			                            Su_matrix(j,i).imag()*Gu_diff_matrix(jsum,isum)
			                   ...
			                   + nfm*(
			                            Gu_diff_matrix(j,i).imag()*Su_matrix(jsum,isum)
			                          + Gu_diff_matrix(jsum,isum).imag()*Su_matrix(j,i)
			                         );
#endif
		}
	}
...
// Hierfuer muss gelten: l>=k
	for(int jsum=jmin+l, jtilde=0, j=jmin; j<=jmax;++j, ++jsum, ++jtilde){ 
		...
		for(int isum=imin+k, itilde=0, i=imin; i<=i1; ++i, ++isum, ++itilde){  
			//cout<<"orts i="<<i<<endl;
			ret(jtilde,itilde) = nf* ( 
			                          Su_syma(j,i).imag()*Gu_diff_syma(jsum,isum)
			                         )
			                   + nfm*(
			                          Gu_diff_syma(j,i).imag()*Su_syma(jsum,isum)
			                         );
		...
		for(int isum=i1+1+k, itilde=i1-imin+1, i=i1+1; i<=i2; ++i, ++isum, ++itilde){  
			//cout<<"orts i="<<i<<endl;
			ret(jtilde,itilde) = nf* ( 
			                          Su_syma(i,j).imag()*Gu_diff_syma(jsum,isum)
			                         )
			                   + nfm*(
			                          Gu_diff_syma(i,j).imag()*Su_syma(jsum,isum)
			                         );
		}
		for(int isum=i2+1+k, itilde=i2-imin+1, i=i2+1; i<=imax; ++i, ++isum, ++itilde){  
			//cout<<"orts i="<<i<<endl;
			ret(jtilde,itilde) = nf* ( 
			                          Su_syma(i,j).imag()*Gu_diff_syma(isum,jsum)
			                         )
			                   + nfm*(
			                          Gu_diff_syma(i,j).imag()*Su_syma(isum,jsum)
			                         );
		}
#else
		for(int isum=imin+k, itilde=0, i=imin; i<=i1; ++i, ++isum, ++itilde){  
			//cout<<"orts i="<<i<<endl;
			ret(jtilde,itilde) = nf* ( 
			                          Su_syma(j,i).imag()*Gu_diff_syma(jsum,isum) + Su_syma(jsum,isum).imag()*Gu_diff_syma(j,i)
			                         )
			                   + nfm*(
			                          Gu_diff_syma(j,i).imag()*Su_syma(jsum,isum) + Gu_diff_syma(jsum,isum).imag()*Su_syma(j,i)
			                         );
		}
		for(int isum=i1+1+k, itilde=i1-imin+1, i=i1+1; i<=i2; ++i, ++isum, ++itilde){  
			//cout<<"orts i="<<i<<endl;
		 	ret(jtilde,itilde) = nf* ( 
			                          Su_syma(i,j).imag()*Gu_diff_syma(jsum,isum) + Su_syma(jsum,isum).imag()*Gu_diff_syma(i,j)
			                         )
			                   + nfm*(
			                          Gu_diff_syma(i,j).imag()*Su_syma(jsum,isum) + Gu_diff_syma(jsum,isum).imag()*Su_syma(i,j)
			                         );
		}
		for(int isum=i2+1+k, itilde=i2-imin+1, i=i2+1; i<=imax; ++i, ++isum, ++itilde){  
			//cout<<"orts i="<<i<<endl;
		 	ret(jtilde,itilde) = nf* ( 
			                          Su_syma(i,j).imag()*Gu_diff_syma(isum,jsum) + Su_syma(isum,jsum).imag()*Gu_diff_syma(i,j)
			                         )
			                   + nfm*(
			                          Gu_diff_syma(i,j).imag()*Su_syma(isum,jsum) + Gu_diff_syma(isum,jsum).imag()*Su_syma(i,j)
			                         );
		}
#endif
	}
...
template<int mode> matrix<double> Integrand_P_bubble_zero_mag<mode>::select(matrix < complex < double > > &M){
	matrix<double> n;
...
template <int mode> class P_bubble_zero_mag{
	public:
		...
		P_bubble_zero_mag(Physics &phy_in, Numerics &num_in, Precomputation_zeromag<mode> &pre_in, Substitution<mode> &sub_in, double Lambda_in, double measure_flow_in);
...
template <int mode> P_bubble_zero_mag<mode>::P_bubble_zero_mag(Physics &phy_in, Numerics &num_in, Precomputation_zeromag<mode> &pre_in, Substitution<mode> &sub_in, double Lambda_in, double measure_flow_in): phy(phy_in), num(num_in), pre(pre_in), sub(sub_in), Lambda(Lambda_in), measure_flow(measure_flow_in), stops_obj(phy, sub, Lambda){}

template <int mode> matrix<complex<double> > P_bubble_zero_mag<mode>::operator()(double external_freq, int l, int k){
	Integrand_P_bubble_zero_mag<mode> P_int(external_freq, phy, num, pre, sub, measure_flow, l, k);  
...
template <int mode> matrix<complex<double> > P_bubble_zero_mag<mode>::operator()(matrix<double> &job){
	return operator()(job(0), (int) job(1), (int) job(2));
}

template <int mode> int P_bubble_zero_mag<mode>::dim_r(matrix<double> &job){
	int l = (int) job(1);
	return num.Nges - abs(l);
}

template <int mode> int P_bubble_zero_mag<mode>::dim_c(matrix<double> &job){
	int k = (int) job(2);
	return num.Nges - abs(k);
}

template <int mode> int P_bubble_zero_mag<mode>::volume(matrix<double> &job){
