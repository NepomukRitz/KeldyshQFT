#ifndef P_FLOW_ZERO_MAG_COMPUTE_RHS_WITH_PAID_23082017
#define P_FLOW_ZERO_MAG_COMPUTE_RHS_WITH_PAID_23082017
...
template<int mode> class P_ud_compute_rhs{
 	public:
		Numerics &num;
		P_bubble_central_zero_mag_extended<mode> &Bubble_dyn;
		Barevertex &barevertex;
		Vertex<mode> &gamma;
		matrix<matrix<double> > &Pud_stat_data;
		matrix<matrix<double> > &Bubble_stat_data;
		P_ud_compute_rhs(Numerics &num_in, P_bubble_central_zero_mag_extended<mode> &Bubble_dyn_in, Barevertex &barevertex_in, Vertex<mode> &gamma_in, matrix<matrix<double> > &Bubble_stat_data, matrix<matrix<double> > &Pud_stat_data_in);
		syma<complex<double> > operator()(int p_freq_ind);
};

template<int mode> P_ud_compute_rhs<mode>::P_ud_compute_rhs(Numerics &num_in, P_bubble_central_zero_mag_extended<mode> &Bubble_dyn_in, Barevertex &barevertex_in, Vertex<mode> &gamma_in, matrix<matrix<double> > &Bubble_stat_data_in, matrix<matrix<double> > &Pud_stat_data_in):num(num_in), Bubble_dyn(Bubble_dyn_in), barevertex(barevertex_in), gamma(gamma_in), Bubble_stat_data(Bubble_stat_data_in), Pud_stat_data(Pud_stat_data_in){}
